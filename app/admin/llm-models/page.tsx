...
import { CreateLlmModelForm } from "./CreateLlmModelForm";
...
export default async function AdminLlmModelsPage({
  ...
  const providers = ((providersData ?? []) as Array<{ id: unknown; name: unknown }>)
    .map((row) => ({
      id: String(row.id ?? ""),
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm">
        ...
        {providersError ? (
          <p className="mt-3 text-sm text-red-700">{providersError.message}</p>
        ) : (
          <>
            <CreateLlmModelForm providers={providers} />
            {providers.length === 0 ? (
              <p className="mt-3 text-sm text-amber-700">
                Create an LLM provider before creating a model.
              </p>
            ) : null}
          </>
        )}
